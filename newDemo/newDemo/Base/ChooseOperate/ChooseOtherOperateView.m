...
#import "ChooseOtherOperateView.h"
#import "ChooseOperateTableViewCell.h"
@implementation ChooseOtherOperateView
{
...
-(void)setView:(NSString *)type contentArr:(NSMutableArray *)listArr currentStr:(NSString *)currentStr{
    ...
    //选择页面名称
    UILabel * headLab = [UILabel labWithFrame:CGRectMake(10, 0, self.frame.size.width-20, 45) text:type font:20 textColor:RGBACOLOR(88,168,249,1) textAlignment:0];
    [self addSubview:headLab];
    //分割线1
    UIView * lineView1 = [UIView viewWithFrame:CGRectMake(0, 43, self.frame.size.width, 2) backgroundColor:RGBACOLOR(66,156,250,1)];
    [self addSubview:lineView1];
    //搜索栏
    UITextField * searchTF = [UITextField textFieldWithFrame:CGRectMake(5, 45, self.frame.size.width-10, 45) palceholder:@"请输入核算项目" font:13 secureTextEntry:NO];
    searchTF.tintColor = LAB3_COLOR;
    [searchTF addTarget:self action:@selector(searchClick:) forControlEvents:UIControlEventEditingChanged];
    [self addSubview:searchTF];
    //分割线2
    UIView * lineView2 = [UIView viewWithFrame:CGRectMake(5, 88, self.frame.size.width-10, 2) backgroundColor:RGBACOLOR(66,156,250,1)];
...
-(void)searchClick:(UITextField *)textField{
    ...
    for (NSString * string in _middleArr) {
        if (string.length >= textField.text.length) {
            if([string rangeOfString:textField.text].location !=NSNotFound)
            {
                [searchArr addObject:string];
            }
        }
    }
...
-(void)creatTableView{
    //创建tableView
    _mainContentListTV = [[UITableView alloc]initWithFrame:CGRectMake(0, 90,self.frame.size.width,self.frame.size.height-90) style:UITableViewStyleGrouped];
